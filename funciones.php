<?php
	...
	function emailExiste($email)
	{
		...
		$stmt = $mysqli->prepare("SELECT id FROM usuarios WHERE correoUsuario = ? LIMIT 1");
	...
	function resultBlock($errors){
		if(count($errors) > 0)
		{
			...
			<a href='index.php' onclick=\"showHide('error');\">[X]</a>
			<ul>";
	...
	function registraUsuario($usuario, $pass_hash, $nombre,$apellido, $email, $activo, $token, $tipo_usuario){
		...
		$stmt = $mysqli->prepare("INSERT INTO usuarios (usuario, password, nombreUsuario,apellidoUsuario,correoUsuario, activacion, token, tipoUsuario) VALUES(?,?,?,?,?,?,?,?)");
	...
	function enviarEmail($email, $nombre, $asunto, $cuerpo){
		...
		$mail->Username = 'solucionesinfortmatica@gmail.com'; //Modificar
		$mail->Password = '18febreroliaM'; //Modificar
		
		$mail->setFrom('solucionesinfortmatica@gmail.com', 'Soluciones Informaticas'); //Modificar
	...
	function login($usuario, $password)
	{
		...
		$stmt = $mysqli->prepare("SELECT id, tipoUsuario, password FROM usuarios WHERE usuario = ? || correoUsuario = ? LIMIT 1");
	...
	function isActivo($usuario)
	{
		...
		$stmt = $mysqli->prepare("SELECT activacion FROM usuarios WHERE usuario = ? || correoUsuario = ? LIMIT 1");
		$stmt->bind_param('ss', $usuario, $usuario);
		$stmt->execute();
		$stmt->bind_result($activacion);
		$stmt->fetch();
